...
function start(){
    ...
    // "Noise" function //
    function Noise(X,Y,f,i){
        var noise = Math.noise(X/f,Y/f)+(Math.noise(X/(f/2),Y/(f/2))/2)+(Math.noise(X/(f/4),Y/(f/2))/4);
    ...
    for (x = -SIZE.x/2; x<SIZE.x/2; x++){
        ...
        var noise = Noise(x+(SIZE.x/2),1,50,0.05);
    ...
    for (const key in BLOCKS){
        ...
        if (BLOCKS[key].image == document.getElementById("Stone")){
            if (Noise(pos.x+(SIZE.x/2),pos.y+(SIZE.y/2),10,0.01) < 0.5){
    ...
    const sprite = new Sprite(new Vector2(0,-100), new Vector2(SPRITE_SIZE,SPRITE_SIZE), document.getElementById("Orc"));
    ...
    function Tick(){
        ...
        sprite.position.y = PLAYER.position.y-2;
        sprite.position.x = PLAYER.position.x-1;
